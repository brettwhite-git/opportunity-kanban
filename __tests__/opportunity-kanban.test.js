...
jest.mock('N/url', () => ({
    resolveScript: jest.fn().mockReturnValue('/app/site/hosting/scriptlet.nl?script=1&deploy=1')
}));
...
beforeEach(() => {
    ...
    queries.buildStatusColumns.mockReturnValue([
        { id: '6', name: 'Proposal' }
    ]);
...
describe('opportunity-kanban portlet', () => {
    ...
    it('includes updateUrl in KANBAN_DATA', () => {
        const portletObj = { title: '', html: '' };
        portlet.render({ portlet: portletObj });
        expect(portletObj.html).toContain('"updateUrl"');
    });
